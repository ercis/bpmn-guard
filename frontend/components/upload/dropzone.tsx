                  ...
                  {file.errors
                    .map((e) =>
                      e.message.startsWith('File is larger than')
                        ? `File is larger than ${formatFileSize(maxFileSize)} (Size: ${formatFileSize(file.size)})`
                        : e.message
                    )
                    .join(', ')}
                </p>
  ...
  return (
    <div className={cn('flex flex-col items-center gap-y-3', className)}>
      ...
      <p className="text-sm text-muted-foreground">
        Drag and drop {!maxFiles || maxFiles > 1 ? 'files' : 'a file'} here, or
      </p>
      <Button
        type="button"
        variant="secondary"
